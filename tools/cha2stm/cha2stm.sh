...
if [ $# -ne 2 ]; then
  echo "Usage: cha2stm.sh <basename>.cha xml_dirname"
  echo
  exit 1;
fi
...
if [ -f $dirname/$basename.cha ]; then
  ...
  ~/bin/lib/zulu8.17.0.3-jdk8.0.102-linux_x64/bin/java -cp ~/bin/lib/chatter.jar org.talkbank.chatter.App -inputFormat cha -outputFormat xml -output $outputdirname/$basename.xml $dirname/$basename.cha
  # Convert CHATTER xml to STM
# parse_cha_xml.py $dirname/$basename.xml --stm --replacement
  parse_cha_xml.py $outputdirname/$basename.xml --stm --oov
fi
