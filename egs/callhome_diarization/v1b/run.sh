...
if [ $stage -le 6 ];then
    ...
    run.pl exp/ivectors_sre/log/plda.log \
...
if [ $stage -le 7 ];then
    ...
    # The first directory contains the PLDA model.  The second directory contains the iVectors
    # for callhome1.
    diarization/score_plda.sh --cmd "$train_cmd --mem 4G" \
			      --nj 20 exp/ivectors_sre exp/ivectors_callhome1 \
			      exp/ivectors_callhome1/plda_scores

    # Do the same thing for callhome2.
    diarization/score_plda.sh --cmd "$train_cmd --mem 4G" \
			      --nj 20 exp/ivectors_sre exp/ivectors_callhome2 \
...
if [ $stage -le 8 ];then
    ...
    diarization/compute_plda_calibration.sh --cmd "$train_cmd --mem 4G" \
					    --nj 20 exp/ivectors_sre exp/ivectors_callhome2 exp/ivectors_callhome2/plda_scores

    diarization/compute_plda_calibration.sh --cmd "$train_cmd --mem 4G" \
					    --nj 20 exp/ivectors_sre exp/ivectors_callhome1 exp/ivectors_callhome1/plda_scores
...
if [ $stage -le 9 ];then
    ...
    # OVERALL SPEAKER DIARIZATION ERROR = 14.42 percent of scored speaker time  `(ALL)
...
if [ $stage -le 10 ];then
    ...
    # OVERALL SPEAKER DIARIZATION ERROR = 13.95 percent of scored speaker time  `(ALL)
