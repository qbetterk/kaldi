...
def VB_diarization(X, m, iE, w, V, sp=None, q=None,
  ...
  ll = (X**2).dot(-0.5*iE.T) + X.dot(iE.T*m.T)-0.5*((iE * m**2 - np.log(iE)).sum(1) - 2*np.log(w+1e-20) + D*np.log(2*np.pi))
  ll *= llScale
  LL = logsumexp(ll, axis=1)
  NN =  np.exp(ll - LL[:,np.newaxis]) * statScale
  ...
  LL = np.sum(LL) # total log-likelihod as calculated using UBM
  mixture_sum = coo_matrix((np.ones(C*D), (np.repeat(range(C),D), range(C*D))))

  # print(NN.shape)
  # print(np.asarray(NN).shape)
  # print((NN.multiply(ll - np.log(w))).shape)
  # print((NN.multiply(ll - np.log(w))))
  # print(np.asarray(NN.multiply(ll - np.log(w))).shape)
  # print(np.asarray(NN.multiply(ll - np.log(w))))
  G = np.sum((NN.multiply(ll - np.log(w+1e-20))), axis=1)  # eq. (15)
  ...
  for ii in range(maxIters):
    ...
    for sid in range(maxSpeakers):
        ...
        # print(sid)
        # print(invL.shape)
        # print(VtiEFs[sid].shape)
        # print(a.shape)
        # print(G.shape)
        # print(NN.shape)
        # print(mixture_sum.shape)
        # print(VtiEF.shape)
        # print(VtiEF.dot(a).shape)
        # print((G+VtiEF.dot(a)).shape)
        lls[:,sid] = G.ravel() + VtiEF.dot(a).ravel() - 0.5 * NN.dot(mixture_sum.dot(((invL+np.outer(a,a)).astype(V.dtype).dot(V) * (iE.flat * V)).sum(axis=0))).ravel()
        L += 0.5 * (logdet(invL) - np.sum(np.diag(invL) + a**2, axis=0) + R)
    ...
    # self-loop probability 'loopProp' and the transition probabilities to the 
    ...
    # for each speaker. 
    q, tll, lf, lb = forward_backward(lls.repeat(minDur,axis=1), tr, tr[::-1].diagonal(), np.arange(1,maxSpeakers+1)*minDur-1)
    #q, tll, lf, lb = forward_backward(lls.repeat(minDur,axis=1), tr, tr[::-1].diagonal())
...
def DER(q, ref, expected=True, xentropy=False):
    ...
    for s in range(err_mx.shape[0]):
        tmpq = q[ref == s,:]
        err_mx[s] = (-np.log(tmpq+1e-20) if xentropy else tmpq).sum(0)
...
def logsumexp(x, axis=0):
    xmax = x.max(axis)
    x = xmax + np.log(np.sum(np.exp(x - np.expand_dims(xmax, axis)), axis)+1e-20)
    # infs = np.isinf(xmax)
    # if np.ndim(x) > 0:
    #   x[infs] = np.xmax[infs]
    # elif infs:
