...
if [ $stage -le 9 ];then
    ...
    cat exp/ivectors_callhome1/plda_scores/rttm exp/ivectors_callhome2/plda_scores/rttm \
        | perl local/md-eval.pl -1 -c 0.25 -r local/fullref.rttm -s - 2> /dev/null | tee > result.txt
...
#    # OVERALL SPEAKER DIARIZATION ERROR = 10.63 percent of scored speaker time  `(ALL)
#
...
#    # OVERALL SPEAKER DIARIZATION ERROR = 9.00 percent of scored speaker time  `(ALL)
...
#    wait
#    # OVERALL SPEAKER DIARIZATION ERROR = 9.45 percent of scored speaker time  `(ALL)
