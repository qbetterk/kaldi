...
while (<META>) {
  ...
  if (-f $call2sph[$call_id]) {
    ...
    print WAV "${utt_A} sph2pipe -f wav -p -c 1 $call2sph[$call_id] | sox - -t wav -r 16k -t wav -|\n";
    print WAV "${utt_B} sph2pipe -f wav -p -c 2 $call2sph[$call_id] | sox - -t wav -r 16k -t wav -|\n";
    $num_good_files++;
  } else {
