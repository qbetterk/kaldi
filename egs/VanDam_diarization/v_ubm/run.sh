...
if [ $stage -le 1 ];then
    ...
    cp -r data/vandam data/train  
fi

if [ $stage -le 2 ];then

    for name in train vandam_trainplda vandam1_test vandam2_test; do
...
if [ $stage -le 5 ];then
    ...
    diarization/train_diag_ubm.sh --cmd "$train_cmd -l mem_free=20G,ram_free=20G" \
				  --nj 40 --num-threads 8 --delta-order 1 data/train $num_components \
