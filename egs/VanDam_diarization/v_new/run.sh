...
if [ $stage -le 2 ]; then
  ...
  sid/train_full_ubm.sh --nj 40 --remove-low-count-gaussians false \
    ...
    data/train exp/diag_ubm_$num_components \
...
if [ $stage -le 3 ]; then
  ...
  diarization/extract_ivectors.sh --cmd "$train_cmd --mem 25G" \
    --nj 40 --window 3.0 --period 1.5 --min-segment 0.05 --apply-cmn false \
  ...
  diarization/extract_ivectors.sh --cmd "$train_cmd --mem 20G" \
    ...
    --min-segment 0.05 exp/extractor_c${num_components}_i${ivector_dim} \
